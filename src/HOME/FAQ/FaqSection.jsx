...
const FaqSection = ({ title, questions, defaultOpen = false }) => {
    return (
        <section className="flex items-center gap-2.5 py-3.75 relative bg-[#f2f1fd] ">
            <details open={defaultOpen} className="w-full group/main open:z-40 relative  ">
                <summary className="text-[16px] text-[#193352] cursor-pointer font-bold uppercase list-none flex items-center gap-1">
                    ...
                    {title}
                    <div className="flex items-start absolute right-0  group-open/main:top-[10%] ">
                        <img
                ...
                <div className="mt-2">
                    {questions.map((question, index) => (
                        <details key={question.question} className="pl-2.5 group/sub w-full">
